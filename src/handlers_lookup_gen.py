...
project_name = "at_url_converter"

modules_names = [name for name in os.listdir(os.path.join("src", project_name, "handlers")) if name.endswith('.py') and not name.startswith('_')]
modules = [importlib.import_module(project_name + ".handlers." + name[:-3]) for name in sorted(modules_names)]
